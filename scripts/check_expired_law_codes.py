...
from qdrant_config import get_collection_name, init_qdrant_from_args, load_env, make_qdrant_client, normalize_chunk_payload
...
def collect_law_codes(client, collection: str, *, scroll_limit: int = 1000) -> dict[str, str]:
    ...
    while True:
        ...
        for point in points:
            ...
            code = (payload.get("law_code") or payload.get("document_number") or "").strip()
            if not code:
                continue
            if code not in codes:
                title = str(payload.get("law_title") or payload.get("document_title") or "").strip()
                codes[code] = title
        total += len(points)
